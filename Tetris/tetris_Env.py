...
class TetrisEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #(prev 100, 130)
        if lines_cleared == 1:
            reward += -4 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)
        elif lines_cleared == 2:
            reward += -4 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)
        elif lines_cleared == 3:
            reward += -4 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)
        elif lines_cleared == 4:
            reward += 70 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)

        if t_spin_type == 2 and lines_cleared == 2:
            reward += 20 # * np.sqrt(self.game.Combo + 1)
        elif t_spin_type == 2 and lines_cleared == 3:    
            reward += 21 # * np.sqrt(self.game.Combo + 1)
        elif t_spin_type == 1:   
