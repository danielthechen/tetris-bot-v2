...
class TetrisEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #reward (prev 5):
        reward -= max(-3, min(0, (blocked_holes - self.history["blocked_holes"])) * 3)
        ...
        reward -= min(max(0,(blocked_holes - self.history["blocked_holes"]) * 4), 16)
        reward -= min(max(0,(row_holes - self.history["row_holes"])) * 2, 6)
        ...
        if lines_cleared == 1:
            ...
            reward += -1.5 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)
        elif lines_cleared == 3:
            reward += -2 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)
        elif lines_cleared == 4:
            reward += 50 # * (0.8 + 0.2 * (20 - place_height) / 20) * np.sqrt(self.game.Combo + 1)
